...
- **E2E scripts (`npm run test:e2e:career`, `npm run test:e2e:talent-mapper`) need the dev server running** and a one-time `npx playwright install --with-deps chromium`. Known caveat: `test:e2e:talent-mapper` currently fails on a stale text assertion (it waits for "publication affiliation is not confirmed", but the evidence dialog now reads "Affiliation reflects publication metadata…" in `components/talent-mapper/CandidateDetail.tsx`). The demo flow itself works; only that assertion is out of date.
