  ...
  if (applications.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-300 bg-white/50 py-20 text-center">
        ...
        <p className="mt-3 text-sm font-medium text-zinc-700">No answers yet</p>
  ...
  return (
    <div className="flex gap-4">
      ...
      <div className="flex-1">
        {!selectedApp ? (
          <div className="flex h-48 items-center justify-center rounded-2xl border border-dashed border-zinc-200">
            <p className="text-sm text-zinc-400">Select an application to see answers</p>
          </div>
        ) : (
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
            <h2 className="mb-1 font-semibold text-zinc-900">{selectedApp.role}</h2>
            <p className="mb-4 text-sm text-zinc-500">{selectedApp.company}</p>
            ...
            {Object.keys(selectedApp.answers).length === 0 ? (
              ...
              <div className="space-y-4">
                {Object.entries(selectedApp.answers).map(([question, answer]) => (
                  <div key={question} className="rounded-xl border border-zinc-100 bg-zinc-50 px-4 py-3">
                    <p className="text-xs font-semibold text-zinc-500">{question}</p>
                    <p className="mt-1 text-sm text-zinc-800">{String(answer)}</p>
                  </div>
                ))}
              </div>
            )}
